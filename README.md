...
- Detecção de colunas (incluindo casos como "Nome SKU", SKU numérico vs. textual, NIVEL1, filtro por Data excluindo Periodo)
...
- `core.js` — toda a lógica de anonimização (portada e testada a partir da versão Python)
- `app.js` — interface: upload, análise automática, filtros, barra de progresso, gravação em streaming
- `lib/` — bibliotecas de terceiros (PapaParse para CSV, SheetJS para Excel), incluídas localmente para não depender de nenhum serviço externo no ar
